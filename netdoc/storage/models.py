...
from sqlalchemy import (
    ...
    Numeric, Date, UniqueConstraint, Index, func,
)
...
class SecurityEvent(Base):
    """Security events generated by NetDoc (new device, vuln, port change, IP conflict).

    Stored locally so users can see security activity directly in NetDoc without
    opening the Wazuh dashboard. Events are also forwarded to Wazuh when configured.

    event_type: new_device | new_vuln | port_change | ip_conflict
    severity:   info | warning | critical
    """
    __tablename__ = "security_events"
    __table_args__ = (
        Index("ix_secevt_device_id", "device_id"),
        Index("ix_secevt_ts",        "ts"),
    )
    ...
    description = Column(Text,       nullable=True)
    details     = Column(JSON,       nullable=True)
    ts          = Column(DateTime,   default=datetime.utcnow, server_default=func.now(), nullable=False)
